...
public:
    ...
    void inference(cv::Mat img)
    {
        // StopWatch time6;
        // time6.tic();
        run(img, {320, 320});                              // 模型推理
        // std::cout<<"time6:"<<time6.toc()<<std::endl;
        render();  
    }
    /**
     * @brief
    ...
    void run(cv::Mat frame,const std::vector<int64_t> &input_size)
    {   
        //preprocess
        // StopWatch time7;
        // time7.tic();
        ...
        cv::Mat x;
        NDTensor scale_factor({1, 2}), image({1, 3, input_size[0], input_size[1]});
        ...
        transposeAndCopyToTensor(x, image);

        this->predictor_nna_->set_inputs({{"image", image}});

        // std::cout<<"time7:"<<time7.toc()<<std::endl;
        ...
        // std::cout<<"time8:"<<time7.toc()<<std::endl;
